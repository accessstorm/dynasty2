...
const BowTies = () => {
  ...
  // Color filters with counts for bow ties
  const colorFilters = [
    { color: 'black', label: 'Black', count: 8, colorCode: '#000000' },
    { color: 'navy', label: 'Navy Blue', count: 6, colorCode: '#172554' },
    ...
    { color: 'red', label: 'Red', count: 2, colorCode: '#b91c1c' },
    { color: 'beige', label: 'Beige', count: 1, colorCode: '#f5f5dc' },
  ];
  
  useEffect(() => {
    // Get products from static service
    const { bowTies } = StaticProductService.getStaticProducts();
    setProducts(bowTies);
    ...
    applyFilters(initialPriceRange, initialColors, initialSort, bowTies);
  }, [location.search]);
  ...
  return (
    <div className="py-8">
      ...
      <Container size="xl" className="mb-6 px-6 md:px-8">
        <Text component="h1" className="text-3xl font-serif text-center mb-2">Bow Ties Collection</Text>
        <Text className="text-center text-gray-600 mb-8">
          Handcrafted bow ties for every occasion
        </Text>
