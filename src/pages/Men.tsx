...
const Men = () => {
  ...
  // Color filters with counts - updated with all required colors
  const colorFilters = [
    { color: 'black', label: 'Black', count: 33, colorCode: '#000000' },
    { color: 'blue', label: 'Blue', count: 25, colorCode: '#1e3a8a' },
    { color: 'grey', label: 'Grey', count: 12, colorCode: '#6b7280' },
    { color: 'navy', label: 'Navy Blue', count: 7, colorCode: '#172554' },
    { color: 'burgundy', label: 'Burgundy', count: 12, colorCode: '#800020' },
    { color: 'green', label: 'Green', count: 9, colorCode: '#166534' },
    { color: 'red', label: 'Red', count: 8, colorCode: '#b91c1c' },
    { color: 'white', label: 'White', count: 15, colorCode: '#ffffff' },
    { color: 'beige', label: 'Beige', count: 5, colorCode: '#f5f5dc' },
    { color: 'brown', label: 'Brown', count: 7, colorCode: '#8b4513' },
    { color: 'purple', label: 'Purple', count: 4, colorCode: '#4a148c' },
    { color: 'pink', label: 'Pink', count: 3, colorCode: '#f472b6' }
  ];
  
  useEffect(() => {
    // Get products from static service
    const { men } = StaticProductService.getStaticProducts();
    setProducts(men);
    ...
    applyFilters(initialPriceRange, initialColors, initialSort, men);
  }, [location.search]);
  ...
  return (
    <div className="py-8">
      ...
      <Container size="xl" className="mb-6 px-6 md:px-8">
        <Text component="h1" className="text-3xl font-serif text-center mb-2">Men's Collection</Text>
        <Text className="text-center text-gray-600 mb-8">
          Neckties designed for the modern man
        </Text>
